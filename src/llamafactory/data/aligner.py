...
def convert_ours(
    ...
    r"""
    Converts ours format dataset to the standard format.
    """
    tag_mapping = {
    ...
    source_messages = example[dataset_attr.source_messages]
    target_messages = example[dataset_attr.target_messages]
    if (
        dataset_attr.system_tag
        and len(source_messages) != 0
        and source_messages[0][dataset_attr.role_tag] == dataset_attr.system_tag
    ):
        system = source_messages[0][dataset_attr.content_tag]
        source_messages = source_messages[1:]
    else: # here
        system = example[dataset_attr.system] if dataset_attr.system else ""

    source_aligned_messages, target_aligned_messages = [], []
    broken_data = False
    for turn_idx, message in enumerate(source_messages):
        if message[dataset_attr.role_tag] not in accept_tags[turn_idx % 2]:
            logger.warning("Invalid role tag in {}.".format(source_messages))
            broken_data = True

        source_aligned_messages.append(
            {"role": tag_mapping[message[dataset_attr.role_tag]], "content": message[dataset_attr.content_tag]}
        )
    
    for turn_idx, message in enumerate(target_messages):
        if message[dataset_attr.role_tag] not in accept_tags[turn_idx % 2]:
            logger.warning("Invalid role tag in {}.".format(target_messages))
            broken_data = True

        target_aligned_messages.append(
            {"role": tag_mapping[message[dataset_attr.role_tag]], "content": message[dataset_attr.content_tag]}
        )

    if (not dataset_attr.ranking and len(source_aligned_messages) % 2 != 0) or (
        dataset_attr.ranking and len(source_aligned_messages) % 2 == 0
    ):
        logger.warning("Invalid message count in {}.".format(messages))
        broken_data = True
    
    if (not dataset_attr.ranking and len(target_aligned_messages) % 2 != 0) or (
        dataset_attr.ranking and len(target_aligned_messages) % 2 == 0
    ):
        logger.warning("Invalid message count in {}.".format(messages))
        broken_data = True
    ...
    if (
        ...
        or source_rejected[dataset_attr.role_tag] not in accept_tags[-1]
    ):
        logger.warning("Invalid role tag in {}.".format([source_chosen, source_rejected]))
        broken_data = True
    
    if (
        target_chosen[dataset_attr.role_tag] not in accept_tags[-1]
        or target_rejected[dataset_attr.role_tag] not in accept_tags[-1]
    ):
    ...
    source_response = [
        ...
        {"role": tag_mapping[source_rejected[dataset_attr.role_tag]], "content": source_rejected[dataset_attr.content_tag]},
    ]

    target_prompt = target_aligned_messages
    target_response = [
        {"role": tag_mapping[target_chosen[dataset_attr.role_tag]], "content": target_chosen[dataset_attr.content_tag]},
        {"role": tag_mapping[target_rejected[dataset_attr.role_tag]], "content": target_rejected[dataset_attr.content_tag]},
    ]
...
def align_dataset(
    ...
    if dataset_attr.formatting == "alpaca":
        ...
        convert_func = partial(convert_ours, dataset_attr=dataset_attr, data_args=data_args)
